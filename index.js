...
function findLargestInteger(integers) {
  ...
  for(const element of integers) {
    if(element.integer > largest) {
      largest = element.integer;
    }
  }
  return largest;
}
...
class Seasons {
   ...
   */
  constructor() {
    this.seasonsArray = ['spring', 'summer', 'fall', 'winter'];
  ...
  next() {
    ...
    return this.seasonsArray[this.currentSeason];
  }
}
...
class Car {
  ...
  drive(distance) {
    if(this.tank === 0) {
      // do not drive
    } else if(this.tank <= distance / this.mpg) {
   ...
   */
  refuel(gallons) {
    if(gallons + this.tank > this.maxTank) {
